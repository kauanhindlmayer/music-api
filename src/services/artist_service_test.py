...
def test_add_artist(in_memory_database):
    artist = Artist(name='Saib', record_label_id=1)
    artist.id = 1
    in_memory_database.append(artist)

    assert len(in_memory_database) == 1
    assert artist.id == 1
    assert artist.name == 'Saib'
...
def test_get_artist_by_id(in_memory_database):
    first_artist = Artist(name='Saib', record_label_id=1)
    first_artist.id = 1
    second_artist = Artist(name='Saib', record_label_id=2)
    second_artist.id = 2
    ...
    assert retrieved_artist.name == 'Saib'
